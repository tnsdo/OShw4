...
static int handle_mmap(int fd, void *addr);
static void handle_munmap(int mapid);
...
syscall_handler (struct intr_frame *f)
{
  ...
  case SYS_MMAP: // 13 (예시 번호, 실제 syscall-nr.h에서 정의된 순서대로)
    {
      int fd;
      void *addr;
      /* 첫 번째 인자(fd) */
      ...
      /* handle_mmap(...) 호출 결과를 eax에 저장 */
      f->eax = handle_mmap(fd, addr);
      break;
    }

  case SYS_MUNMAP: // 14 (예시 번호)
    {
      int mapid;
      /* 첫 번째이자 유일한 인자(mapid) */
      memread_user(f->esp + 4, &mapid, intsize);
      handle_munmap(mapid);
      break;
    }
...
struct file *
process_get_file_by_fd(int fd) {
  struct thread *t = thread_current ();
  struct file_desc *desc = find_file_desc(t, fd);
  if (desc == NULL)
    return NULL;
  return desc->file;
}
