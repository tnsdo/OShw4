              ...
              uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
  ...
  while (read_bytes > 0 || zero_bytes > 0)
    {
      /* 1) 한 페이지에서 읽어올 바이트와 제로로 채울 바이트 계산 */
      size_t page_read_bytes = read_bytes < PGSIZE ? read_bytes : PGSIZE;
      size_t page_zero_bytes = PGSIZE - page_read_bytes;

      /* --------------------------------------------------------------- */
      /* 아래는 원래 “즉시 palloc → file_read → install_page” 로직입니다. */
      /*
      ...
      */
      ...
      /* 2) 대신 여기서 SPT에만 “file-backed” 엔트리 등록 */
      if (!sup_page_install (
            thread_current (),    /* 현재 스레드 */
            upage,                /* 얼라인된 유저 가상 주소 */
            PAGE_FILE,            /* 파일 기반 페이지 유형 */
            file,                 /* 실행 파일 핸들 */
            ofs,                  /* 파일 오프셋 */
            page_read_bytes,      /* 읽어야 할 바이트 수 */
            page_zero_bytes,      /* 0으로 채울 바이트 수 */
            writable              /* 쓰기 가능 여부 */
          ))
        {
          return false;
